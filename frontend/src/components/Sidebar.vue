  ...
  >
    <div class="bg-white dark:bg-dark-card rounded-lg shadow-sm h-full relative" :class="{ 'rounded-r-none': isCollapsed }">
      ...
      <div class="normal-content p-4 h-full overflow-y-auto">
        ...
        <!-- 侧边栏顶部工具栏 -->
        <div class="mb-3 bg-gray-800/80 p-2 rounded-lg flex items-center justify-between">
          <div class="flex items-center space-x-2">
            <div class="relative">
              <!-- 学习模式切换按钮 -->
              <button 
                @click="toggleLearningModeMenu"
                class="px-2 py-1 text-xs rounded-md flex items-center bg-blue-700/60 text-white hover:bg-blue-600/60 transition-colors"
              >
                <i class="fas" :class="learningModeIcons[currentLearningMode]"></i>
                <span class="ml-1.5">{{learningModeLabels[currentLearningMode]}}</span>
                <i class="fas fa-chevron-down ml-1.5 text-[8px]"></i>
              </button>
              
              <!-- 学习模式菜单 -->
              <div v-if="showLearningModeMenu" class="absolute left-0 top-full mt-1 bg-gray-800 rounded-md shadow-lg z-20 w-48 p-2 border border-gray-700">
                <div 
                  v-for="(label, mode) in learningModeLabels" 
                  :key="mode"
                  @click="setLearningMode(mode)"
                  class="p-1.5 text-xs rounded cursor-pointer hover:bg-gray-700 flex items-center"
                  :class="{'bg-blue-800/50': currentLearningMode === mode}"
                >
                  <i class="fas mr-1.5 text-blue-400" :class="learningModeIcons[mode]"></i>
                  <span>{{label}}</span>
                  <i v-if="currentLearningMode === mode" class="fas fa-check ml-auto text-green-400 text-[8px]"></i>
                </div>
              </div>
            </div>
            ...
            <button 
              ...
              class="w-8 h-6 flex items-center justify-center text-xs rounded bg-gray-700 hover:bg-gray-600 transition-colors"
              :class="{'bg-purple-700 hover:bg-purple-600': isFocusMode}"
              :title="isFocusMode ? '退出专注模式' : '专注模式'"
            >
              <i class="fas" :class="isFocusMode ? 'fa-eye' : 'fa-eye-slash'"></i>
            </button>
          </div>
          
          <!-- 仪表盘设置按钮 -->
          <button 
            @click="showDashboardSettings = !showDashboardSettings"
            class="px-2 py-1 text-xs rounded-md flex items-center bg-gray-700 text-gray-300 hover:bg-gray-600 transition-colors"
            :class="{'bg-blue-700 text-white': showDashboardSettings}"
          >
        ...
        <transition name="slide-fade">
          <div v-if="showDashboardSettings" class="mb-4 bg-gray-800/80 p-3 rounded-lg border border-gray-700/50">
            ...
            <!-- 区域显示切换 -->
            <div class="mb-2">
              <div class="text-xs text-gray-400 mb-1">显示区域</div>
              <div class="grid grid-cols-2 gap-2">
                <div 
                  v-for="(section, key) in sidebarSections" 
                  :key="key"
                ...
                >
                  ...
                  />
                  <label :for="key" class="text-xs text-gray-300 cursor-pointer">{{section.label}}</label>
                </div>
              </div>
            </div>
            ...
            <!-- 区域排序 -->
            <div>
              <div class="text-xs text-gray-400 mb-1">区域顺序 (拖动排序)</div>
              <div class="space-y-1.5">
                <div 
                  v-for="(section, key) in visibleSections" 
                  :key="key"
                  class="flex items-center bg-gray-700/70 p-1.5 rounded cursor-move"
                >
                  <i class="fas fa-grip-lines text-gray-500 mr-2"></i>
                  <span class="text-xs text-gray-300">{{section.label}}</span>
                  <div class="flex items-center ml-auto">
                    <button 
                      @click="moveSectionUp(key)" 
                      class="text-xs text-gray-400 hover:text-blue-400 p-1"
                    >
                      <i class="fas fa-arrow-up"></i>
                    </button>
                    <button 
                      @click="moveSectionDown(key)" 
                      class="text-xs text-gray-400 hover:text-blue-400 p-1"
                    >
            ...
            <div class="mt-3 pt-3 border-t border-gray-700">
              <div class="text-xs text-gray-400 mb-2">主题</div>
              <div class="flex space-x-2">
                <button 
                  v-for="theme in ['默认', '专注', '柔和', '高对比度']" 
                  :key="theme"
                  class="px-2 py-1 text-xs rounded bg-gray-700 text-gray-300 hover:bg-gray-600 transition-colors"
                  :class="{'bg-blue-700 text-white': currentTheme === theme}"
            ...
            <div class="mt-3 flex justify-end">
              <button 
                @click="saveSettings"
                class="px-3 py-1 text-xs rounded bg-green-700 text-white hover:bg-green-600 transition-colors"
              >
                保存设置
              </button>
            </div>
        ...
        <div class="mb-6">
          <div class="flex justify-between items-center mb-3">
            ...
            <!-- 增加视图切换按钮 -->
            <div class="flex items-center">
              <div class="view-toggle mr-3 bg-gray-800 rounded-md p-0.5 flex">
                <button 
                  @click="historyViewMode = 'list'"
                  class="view-toggle-btn flex items-center justify-center text-xs py-1 px-2 rounded"
                  :class="{'bg-blue-700 text-white': historyViewMode === 'list', 'text-gray-400': historyViewMode !== 'list'}"
                >
                ...
                <button 
                  ...
                  class="view-toggle-btn flex items-center justify-center text-xs py-1 px-2 rounded"
                  :class="{'bg-blue-700 text-white': historyViewMode === 'timeline', 'text-gray-400': historyViewMode !== 'timeline'}"
                >
          ...
          <div v-if="historyViewMode === 'timeline'" class="history-timeline mt-3">
            <div class="relative pl-6 border-l-2 border-gray-700">
              <div v-for="(book, index) in filteredReadingHistory" :key="`timeline-${book.id}`" class="mb-4 relative">
                ...
                <!-- 时间线内容 -->
                <div class="bg-gray-800/60 rounded-lg overflow-hidden">
                  <!-- 书籍信息 -->
                  <div class="p-2.5 flex items-center hover:bg-gray-700/40 transition-colors duration-200">
                    <div class="flex-grow">
                      <div class="text-sm font-medium text-blue-200 mb-1">{{book.title}}</div>
                      <div class="flex items-center justify-between text-xs">
                        <span class="text-gray-400">阅读于: {{formatRelativeTime(book.lastReadTime)}}</span>
                        <span class="text-gray-300">进度: <span :class="{
                          'text-red-400': book.readingProgress <= 25,
                          'text-yellow-400': book.readingProgress > 25 && book.readingProgress <= 50,
                          'text-blue-400': book.readingProgress > 50 && book.readingProgress <= 75,
                          'text-green-400': book.readingProgress > 75
                        }">{{book.readingProgress}}%</span></span>
                      </div>
                    </div>
                  ...
                  <!-- 里程碑指示器(如果有) -->
                  <div v-if="book.milestones && book.milestones.length > 0" class="bg-blue-900/20 border-t border-gray-700 px-2.5 py-1.5">
                    <div class="text-xs text-blue-300 mb-1">里程碑</div>
                    <div class="flex flex-wrap gap-1">
                      <span 
                        v-for="milestone in book.milestones" 
                        :key="milestone.id"
                        class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] bg-blue-800/50 text-blue-300"
                      >
                        <i class="fas fa-flag text-amber-400 mr-1"></i>
                        {{milestone.title}}
                      </span>
                    </div>
                  </div>
          ...
          <!-- 列表视图 -->
          <div v-if="historyViewMode === 'list'" class="space-y-2">
            <div 
            ...
            >
              ...
              >
                ...
                <!-- 阅读统计 -->
                <div class="mt-2 pt-2 border-t border-gray-700 flex items-center justify-between text-[10px] text-gray-400">
        ...
        <div class="mb-6">
          ...
          <div class="bg-gray-800/70 p-3 rounded-lg mb-3">
            ...
            <div class="space-y-2">
              <div 
                ...
                :key="concept.id"
                class="flex items-center justify-between group cursor-pointer hover:bg-gray-700/30 transition-colors p-1 rounded relative"
                @click="viewConceptDetails(concept)"
              >
                <div class="flex items-center">
                  <div class="concept-icon w-6 h-6 rounded-full flex items-center justify-center mr-2" 
                    :class="{
                      'bg-green-800/50 text-green-400': concept.masteryLevel >= 80,
                      'bg-blue-800/50 text-blue-400': concept.masteryLevel >= 60 && concept.masteryLevel < 80,
                      'bg-yellow-800/50 text-yellow-400': concept.masteryLevel >= 30 && concept.masteryLevel < 60,
                      'bg-red-800/50 text-red-400': concept.masteryLevel < 30
                    }"
                  >
                    <i class="fas fa-brain text-[10px]"></i>
                  </div>
                  <div>
                    <div class="text-xs text-gray-200">{{concept.name}}</div>
                    <div class="text-[10px] text-gray-400">掌握度: {{concept.masteryLevel}}%</div>
                  </div>
                </div>
                <div class="flex items-center">
                  <span class="text-[10px] px-1.5 py-0.5 rounded-full mr-1.5" 
                    :class="{
                      'bg-green-900/50 text-green-400': concept.masteryLevel >= 80,
                      'bg-blue-900/50 text-blue-400': concept.masteryLevel >= 60 && concept.masteryLevel < 80,
                      'bg-yellow-900/50 text-yellow-400': concept.masteryLevel >= 30 && concept.masteryLevel < 60,
                      'bg-red-900/50 text-red-400': concept.masteryLevel < 30
                    }"
                  >
                    {{concept.status}}
                  </span>
                  ...
                  <div class="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button class="text-xs text-purple-400 hover:text-purple-300 transition-colors">
                ...
                <!-- 掌握度详情浮层 - 悬停时显示 -->
                <div class="concept-details hidden group-hover:block absolute left-0 top-full mt-2 w-48 p-2 bg-gray-800 rounded-lg border border-gray-700 shadow-lg z-20">
                  <div class="text-xs font-medium text-blue-300 mb-1.5">{{concept.name}} 掌握情况</div>
                  <div class="space-y-1">
                  ...
                  <div class="mt-2 pt-1.5 border-t border-gray-700 flex items-center justify-between">
                    <div class="text-[10px] text-blue-400">最近练习: 2天前</div>
                    <button class="text-[10px] bg-purple-700/70 text-white px-1.5 py-0.5 rounded hover:bg-purple-600/70 transition-colors">
                      练习
                    </button>
                  </div>
                </div>
            ...
            <div class="flex justify-between mt-3">
              ...
              <button class="text-xs flex items-center text-purple-400 hover:text-purple-300 transition-colors" @click="showRecommendedExercises = !showRecommendedExercises">
                <i class="fas fa-dumbbell mr-1.5"></i>推荐练习
              </button>
            </div>
            
            <!-- 推荐练习面板 -->
            <transition name="slide-fade">
              <div v-if="showRecommendedExercises" class="mt-3 p-2.5 bg-gray-700/50 rounded-lg border border-gray-700/50">
                <div class="text-xs text-gray-200 mb-2 flex items-center justify-between">
                  <span>针对性练习推荐</span>
                  <button @click="showRecommendedExercises = false" class="text-gray-400 hover:text-gray-200">
                    <i class="fas fa-times"></i>
                  </button>
                </div>
                <div class="space-y-2">
                  <div class="exercise-item p-1.5 bg-gray-800/70 rounded group hover:bg-gray-700/70 transition-colors cursor-pointer">
                    <div class="flex items-center">
                      <div class="w-7 h-7 rounded-full bg-purple-800/50 text-purple-400 flex items-center justify-center mr-2">
                        <i class="fas fa-puzzle-piece"></i>
                      </div>
                      <div>
                        <div class="text-xs text-blue-300">量子纠缠练习</div>
                        <div class="text-[10px] text-gray-400">针对性: 理解能力</div>
                      </div>
                      <div class="ml-auto text-[10px] bg-purple-900/50 text-purple-300 px-1.5 py-0.5 rounded">
                        5分钟
                      </div>
                    </div>
                  </div>
                  <div class="exercise-item p-1.5 bg-gray-800/70 rounded group hover:bg-gray-700/70 transition-colors cursor-pointer">
                    <div class="flex items-center">
                      <div class="w-7 h-7 rounded-full bg-blue-800/50 text-blue-400 flex items-center justify-center mr-2">
                        <i class="fas fa-tasks"></i>
                      </div>
                      <div>
                        <div class="text-xs text-blue-300">量子算法入门</div>
                        <div class="text-[10px] text-gray-400">针对性: 基础概念</div>
                      </div>
                      <div class="ml-auto text-[10px] bg-blue-900/50 text-blue-300 px-1.5 py-0.5 rounded">
                        10分钟
                      </div>
                    </div>
                  </div>
                ...
                <div class="text-center mt-2">
                  <button class="text-[10px] bg-purple-700 text-white px-2 py-1 rounded hover:bg-purple-600 transition-colors">
                    查看全部推荐
                  </button>
                </div>
              </div>
          ...
          <div class="bg-gray-800/70 p-3 rounded-lg">
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-xs text-blue-400 hover:text-blue-300 transition-colors flex items-center" @click="showGoalSettings = !showGoalSettings">
            ...
            <div class="mt-3 pt-3 border-t border-gray-700">
              <div class="flex items-center justify-between mb-2">
                <div class="text-xs text-gray-400">社区进度对比</div>
              ...
              <div class="flex items-center justify-between space-x-2">
                <!-- 进度位置指示 -->
                <div class="flex-grow h-2 bg-gray-700 rounded-full relative">
                  <!-- 社区平均线 -->
                  <div class="absolute top-0 bottom-0 w-px bg-white/50 left-1/2"></div>
                  <!-- 个人位置指示 -->
                  <div class="absolute top-0 bottom-0 w-1.5 h-3 bg-blue-500 rounded-full" style="left: 65%; top: -2px;"></div>
                  <!-- 最高进度指示 -->
                  <div class="absolute top-0 bottom-0 w-px bg-green-500/50" style="left: 80%;"></div>
                </div>
                <div class="text-[10px] text-gray-400">
                  <span class="text-blue-400">超过65%</span>
                </div>
              </div>
              <div class="text-[10px] text-gray-500 mt-1.5 flex justify-between">
                <span>开始</span>
                <span>平均水平</span>
                <span>领先者</span>
              </div>
...
// 学习模式
const currentLearningMode = ref('normal');
...
const learningModeIcons = {
  normal: 'fa-book',
  focus: 'fa-eye',
  review: 'fa-sync-alt',
  explore: 'fa-compass'
};
const showLearningModeMenu = ref(false);
...
// 推荐练习
const showRecommendedExercises = ref(false);

// 成就动画和详情
const showAchievementDetails = (achievement) => {
  currentAchievement.value = achievement.title;
  // 这里可以添加显示详情的逻辑
};

// 学习模式切换
const toggleLearningModeMenu = () => {
  showLearningModeMenu.value = !showLearningModeMenu.value;
};

const setLearningMode = (mode) => {
  currentLearningMode.value = mode;
  showLearningModeMenu.value = false;
  
  // 当选择专注模式时，自动开启专注模式
  if (mode === 'focus') {
    isFocusMode.value = true;
  } else if (isFocusMode.value && mode !== 'focus') {
    // 如果从专注模式切换到其他模式，自动关闭专注模式
...
const saveSettings = () => {
  ...
  // localStorage.setItem('sidebarSettings', JSON.stringify({
...
const moveSectionUp = (key) => {
  const currentOrder = sidebarSections.value[key].order;
  const prevSection = Object.entries(sidebarSections.value)
    .filter(([_, section]) => section.visible && section.order < currentOrder)
    .sort((a, b) => b[1].order - a[1].order)[0];
  
  if (prevSection) {
    const prevKey = prevSection[0];
    const prevOrder = prevSection[1].order;
    sidebarSections.value[key].order = prevOrder;
    sidebarSections.value[prevKey].order = currentOrder;
  }
};

const moveSectionDown = (key) => {
  const currentOrder = sidebarSections.value[key].order;
  const nextSection = Object.entries(sidebarSections.value)
    .filter(([_, section]) => section.visible && section.order > currentOrder)
    .sort((a, b) => a[1].order - b[1].order)[0];
  
  if (nextSection) {
    const nextKey = nextSection[0];
    const nextOrder = nextSection[1].order;
    sidebarSections.value[key].order = nextOrder;
    sidebarSections.value[nextKey].order = currentOrder;
  }
};

// 模拟触发成就动画
const triggerAchievementAnimation = (achievement) => {
  if (achievement) {
    currentAchievement.value = achievement.title;
  }
  showAchievementAnimation.value = true;
